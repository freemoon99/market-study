...
def log(message, **fields):
    ...
    print(f"[market-study] {stamp} {message}{(' ' + suffix) if suffix else ''}", flush=True)
...
def init_db():
    log("db:init:start", engine="postgres" if USE_POSTGRES else "sqlite")
    with db() as conn:
    ...
    log("db:init:done", engine="postgres" if USE_POSTGRES else "sqlite")
...
def enrich_and_store(date_iso):
    ...
    try:
        ...
        log("refresh:source:naver-ranked:done", date=date_iso, rows=len(normalized))
    except Exception as exc:
        log("refresh:source:naver-ranked:failed", date=date_iso, error=repr(exc))
    ...
    limit_count = sum(1 for item in selected if "상한가" in item["tags"])
    volume_count = sum(1 for item in selected if "거래량 1000만주" in item["tags"])
    log("refresh:filter:done", date=date_iso, selected=len(selected), limit_up=limit_count, high_volume=volume_count)

    with db() as conn:
        log("refresh:db:load-existing:start", date=date_iso)
        ...
        log("refresh:db:load-existing:done", date=date_iso, rows=len(existing_by_code))
        conn.execute(
            """
        ...
        for idx, item in enumerate(selected, start=1):
            ...
            try:
                ...
                log("refresh:item:chart:done", date=date_iso, code=item["code"], candles=len(chart))
                for c in chart:
                    conn.execute(
                        """
                ...
                log(
                    "refresh:item:stored",
                    date=date_iso,
                    index=f"{idx}/{len(selected)}",
                    code=item["code"],
                    elapsed=f"{time.time() - item_started:.2f}s",
                )
                time.sleep(0.08)
            except Exception as exc:
                log("refresh:item:error", date=date_iso, index=f"{idx}/{len(selected)}", code=item["code"], name=item["name"], error=repr(exc))
                traceback.print_exc()
        log("refresh:db:replace:done", date=date_iso, rows=len(selected))
    log("refresh:done", date=date_iso, elapsed=f"{time.time() - started:.2f}s", selected=len(selected))
...
def get_day(date_iso):
    started = time.time()
    log("day:get:start", date=date_iso)
    with db() as conn:
        ...
        log("day:get:done", date=date_iso, stocks=len(result), elapsed=f"{time.time() - started:.2f}s")
        return payload


def update_stock(date_iso, code, payload):
    log("stock:update:start", date=date_iso, code=code)
    with db() as conn:
        conn.execute(
    ...
    log("stock:update:done", date=date_iso, code=code, reasons=len(payload.get("reasons", [])))
    return {"ok": True}
...
def export_markdown(date_iso):
    started = time.time()
    log("export:start", date=date_iso)
    ...
    markdown = "\n".join(lines)
    log("export:done", date=date_iso, filename=filename, bytes=len(markdown.encode("utf-8")), elapsed=f"{time.time() - started:.2f}s")
    return {"ok": True, "filename": filename, "markdown": markdown}


def sample_day(date_iso):
    log("sample:create:start", date=date_iso)
    now = dt.datetime.now().isoformat(timespec="seconds")
    sample_chart = []
    base = 10000
    for i in range(40):
    ...
    data = get_day(date_iso)
    log("sample:create:done", date=date_iso, stocks=data["counts"]["total"])
    return data
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if not parsed.path.startswith("/api/"):
            return super().do_GET()
        started = time.time()
        log("http:get:start", path=parsed.path, query=parsed.query or "-")
        try:
            query = urllib.parse.parse_qs(parsed.query)
            if parsed.path == "/api/dates":
                with db() as conn:
                    dates = [r["date"] for r in conn.execute("select date from days order by date desc").fetchall()]
                self.send_json({"today": today_kr(), "dates": dates})
                log("http:get:done", path=parsed.path, status=200, dates=len(dates), elapsed=f"{time.time() - started:.2f}s")
                return
            if parsed.path == "/api/day":
                date_iso = query.get("date", [today_kr()])[0]
                data = get_day(date_iso)
                self.send_json(data)
                log("http:get:done", path=parsed.path, status=200, date=date_iso, stocks=data["counts"]["total"], elapsed=f"{time.time() - started:.2f}s")
                return
            if parsed.path == "/api/export":
                date_iso = query.get("date", [today_kr()])[0]
                data = export_markdown(date_iso)
                self.send_json(data)
                log("http:get:done", path=parsed.path, status=200, date=date_iso, filename=data["filename"], elapsed=f"{time.time() - started:.2f}s")
                return
            self.send_json({"error": "not found"}, 404)
            log("http:get:done", path=parsed.path, status=404, elapsed=f"{time.time() - started:.2f}s")
        except Exception as e:
            log("http:get:error", path=parsed.path, error=repr(e), elapsed=f"{time.time() - started:.2f}s")
            traceback.print_exc()
    ...
    def do_POST(self):
        parsed = urllib.parse.urlparse(self.path)
        started = time.time()
        log("http:post:start", path=parsed.path)
        try:
            payload = self.read_json()
            if parsed.path == "/api/refresh":
                date_iso = payload.get("date") or today_kr()
                log("http:post:refresh:payload", date=date_iso, sample_on_fail=payload.get("sampleOnFail", True))
                try:
                    data = enrich_and_store(date_iso)
                    self.send_json(data)
                    log("http:post:done", path=parsed.path, status=200, date=date_iso, stocks=data["counts"]["total"], elapsed=f"{time.time() - started:.2f}s")
                    return
                except (urllib.error.URLError, TimeoutError, RuntimeError, UnicodeDecodeError) as e:
                    log("http:post:refresh:collect-failed", date=date_iso, error=repr(e))
                    if payload.get("sampleOnFail", True):
                        data = sample_day(date_iso)
                        data["warning"] = f"실데이터 수집 실패로 샘플 데이터를 표시했습니다: {e}"
                        self.send_json(data)
                        log("http:post:done", path=parsed.path, status=200, date=date_iso, fallback="sample", elapsed=f"{time.time() - started:.2f}s")
                        return
                    raise
            if parsed.path.startswith("/api/stocks/"):
                parts = parsed.path.split("/")
                if len(parts) >= 5:
                    data = update_stock(parts[3], parts[4], payload)
                    self.send_json(data)
                    log("http:post:done", path=parsed.path, status=200, date=parts[3], code=parts[4], elapsed=f"{time.time() - started:.2f}s")
                    return
            self.send_json({"error": "not found"}, 404)
            log("http:post:done", path=parsed.path, status=404, elapsed=f"{time.time() - started:.2f}s")
        except Exception as e:
            log("http:post:error", path=parsed.path, error=repr(e), elapsed=f"{time.time() - started:.2f}s")
            traceback.print_exc()
            self.send_json({"error": str(e)}, 500)


def main():
    log("app:boot:start", render=bool(os.environ.get("RENDER")), database_url=bool(os.environ.get("DATABASE_URL")))
    init_db()
    ...
    log("app:boot:done", host=host, port=port, public_url=f"http://{shown_host}:{port}")
    try:
